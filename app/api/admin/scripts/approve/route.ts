...
import { approveCommunityScript } from "../../../../../lib/scripts/review";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await approveCommunityScript(body);

    return NextResponse.json({ ok: true, result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to approve script.";
