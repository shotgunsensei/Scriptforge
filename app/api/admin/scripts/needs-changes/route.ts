...
import { markCommunityNeedsChanges } from "../../../../../lib/scripts/review";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const script = await markCommunityNeedsChanges(body);

    return NextResponse.json({ ok: true, script });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to mark needs changes.";
